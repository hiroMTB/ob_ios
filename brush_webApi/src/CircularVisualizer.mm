...
void CircularVisualizer::composePlotData(vector<BrushData> & data, unordered_map<TYPE, float> & radiusData){
    ...
    for(int i=0; i<data.size(); i++){
        ...
        PlotData & p = s.plot[TYPE::HOUR];
        p.radius     = radiusData[TYPE::HOUR] * ofGetWidth()/2;
        p.size       = MAX(p.radius*0.01f, 2);  // thickness
        p.stAngle    = startDeg;
        p.endAngle   = endDeg;
    ...
    for(int i=0; i<data.size(); i++){
        BrushData & s = data[i];
        PlotData & p = s.plot[TYPE::DAY];
        p.radius = radiusData[TYPE::DAY] * ofGetWidth()/2;
...
void CircularVisualizer::draw_hour(float scale, const vector<BrushData> & data, int num){
    ...
    for(int i=0; i<num; i++){
        ...
        const PlotData & p = s.plot.at(TYPE::HOUR);
...
void CircularVisualizer::draw_day(float scale, const vector<BrushData> & data, int num){
    ...
    for(int i=0; i<num; i++){
        const BrushData & s = data[i];
        const PlotData & p = s.plot.at(TYPE::DAY);
...
void CircularVisualizer::draw_week(float scale, const vector<BrushData> & data, int num){
    ...
    for(int i=0; i<num; i++){
        const BrushData & s = data[i];
        const PlotData & p = s.plot.at(TYPE::WEEK);
        float posx = p.stPos.x;
        float posy = p.stPos.y;
        float size = p.size;
...
void CircularVisualizer::draw_month(float scale, const vector<BrushData> & data, int num){
    ...
    for(int i=0; i<num; i++){
        const BrushData & s = data[i];
        const PlotData & p = s.plot.at(TYPE::MONTH);
        float posx = p.stPos.x;
        float posy = p.stPos.y;
        float size = p.size;
...
void CircularVisualizer::draw_year(float scale, const vector<BrushData> & data, int num){
    ...
    for(int i=0; i<num; i++){
        const BrushData & s = data[i];
        const PlotData & p = s.plot.at(TYPE::YEAR);
        float posx = p.stPos.x;
        float posy = p.stPos.y;
        float size = p.size;
